...
/**
 * Sync nudge: writes a tiny timestamp source file (`src/data/lovable-sync-trigger.ts`) on
 ...
 * real content change. This intentionally lives under `src/` rather than
 * `.lovable/`, because hidden/project metadata paths may be ignored by some
 * sync filters.
 *
...
export async function nudgeSync(reason: string): Promise<{ ok: boolean; commitSha?: string; error?: string }> {
  const triggerPath = "src/data/lovable-sync-trigger.ts";
  try {
